...
export async function trade(tokenAddr, trader, isBuy, tokenAmount, omaxAmount, txhash, comment, timestamp) {
  const result = await axiosPrivate.post(`/token/trade`, {
    tokenAddr, trader, isBuy, tokenAmount, omaxAmount, txhash, comment, timestamp
  });
  return result.data;
}
